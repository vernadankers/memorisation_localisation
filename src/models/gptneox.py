...
class GPTNeoXAttention(nn.Module):
    ...
    def _merge_heads(cls, tensor, num_attention_heads, attn_head_size):
        ...
        # -> [bs, seq_len, num_attention_heads, attn_head_size]
        tensor = tensor.view(tensor.size(0), tensor.size(
        ...
        if query_length == kv_seq_len:
            query_layer = index_first_axis(
                query_layer.reshape(batch_size * kv_seq_len,
                                    self.num_attention_heads, head_dim), indices_k
            )
...
def attention_mask_func(attention_scores, ltor_mask):
    attention_scores.masked_fill_(
        ~ltor_mask, torch.finfo(attention_scores.dtype).min)
...
class GPTNeoXModel(GPTNeoXPreTrainedModel):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList([GPTNeoXLayer(config)
...
class GPTNeoXForSequenceClassification(GPTNeoXPreTrainedModel):
    def __init__(self, config):
        ...
        self.classifier = nn.Linear(
            config.hidden_size, self.num_labels, bias=True)
        self.classifier2 = nn.Linear(
            config.hidden_size, self.num_labels2, bias=True)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                if auxiliary:
                    loss = loss_fct(
                        pooled_logits.view(-1, self.num_labels2), labels.view(-1))
                else:
                    loss = loss_fct(
