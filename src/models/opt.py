...
class OPTForSequenceClassification(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        ...
        # CHANGED bias=False to bias=True December 19, 2023, VDankers
        # CHANGED score to classifier December 19, 2023, VDankers
        self.classifier = nn.Linear(config.word_embed_proj_dim,
                                    self.num_labels, bias=True)
        self.classifier2 = nn.Linear(config.word_embed_proj_dim,
                                     self.num_labels2, bias=True)
    ...
    def forward(
        ...
        if self.cog_training:
            hidden_states = torch.stack(
                transformer_outputs.hidden_states[1:], dim=-1)
            bsz, seqlength, embdim, layers = hidden_states.shape
            hidden_states = hidden_states.reshape(
                bsz * seqlength, embdim, layers)
            hidden_states = self.cog_weights(
                hidden_states)
            hidden_states = hidden_states.reshape(bsz, seqlength, embdim)

        if auxiliary:
            logits = self.classifier2(hidden_states)
        else:
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression":
                ...
                loss = loss_fct(
                    pooled_logits.view(-1, self.num_labels if not auxiliary else self.num_labels2), labels.view(-1))
